...
int main()
{
    ...
    for (i=0;i<strlen(riga);i++)//scandisco tutta la riga
    {
        if(riga[i]!=' '&&riga[i]!='\0')//se il carattere è diverso da spazio o \0 lo copio in un vettore di caratteri
        {
            parola[c]=riga[i];
            c++;
            if (riga[i+1]==' '||riga[i+1]=='\0')//se arrivo a prima di uno spiazio o fine della riga ho trovato una parola
            {
...
void modifica_parola(char parola[N],dizionario righe[L], int nr)
{
  int indice=-1,flag=0,indice_inizio,indice_fine,np;
  int controllo=0;//serve per ricercare gli indici
  char s1[N],s2[N];//sfrutto due stringhe per codificare il tutto.
  for(int np=0;np<nr;np++)
  {
      if(strstr(parola,righe[np].caratteri)!=NULL)//trovata una corrispondenza
      {
          indice=np;
          flag=0;
          ...
          {
          controllo=0;//diventa 0 prima di ogni ricerca della corrispondenza tra caratteri
          for(int j=i;j<strlen(righe[indice].caratteri)+i;j++)//ricerco caratteri adiacenti tra loro andando avanti nella parola
          {
          ...
          controllo=0;
           ...
           if(i>indice_fine)//copio su s2 i caratteri successivi all'ultimo da sostituire
           {
           ...
           strcpy(parola,s1);//copio s1 che contiene la parola codificata nella "parola"
       }
  }
